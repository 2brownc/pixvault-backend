...
export async function getImages(
  ...
  if (!access_token) {
    throw new Error("No Open Verse access_token; Cannot get images.");
  }
  ...
  try {
    ...
    throw new Error(`Cannot to get images: ${error}`);
  }
...
export async function getImagesInfo(identifiers: ImageId[]): Promise<Image[]> {
  ...
  for (const identifier of identifiers) {
    try {
      ...
      /*
        make sure API is improved so
        list of failed images get sent to the requester
      */
      console.error(`Failed to get image info for ${identifier}: ${error}`);
    }
  }
